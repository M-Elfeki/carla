...
class LSTM_ShareH(nn.ModuleList):
    def __init__(self, state_dim=3, hidden_dim=256):
        super(LSTM_ShareH, self).__init__()
        self.state_dim = state_dim
        self.hidden_dim = hidden_dim

        self.lstm1_kp = nn.LSTMCell(input_size=state_dim, hidden_size=hidden_dim)
    ...
    def forward(self, x, y, infer=False):
        ...
        for batch_idx in range(len(x)):
            ...
            for input_t_idx in range(len(cur_seq)):
                ...
                h_t, c_t = self.lstm1_locs(input_centers_t, (h_t, c_t))
                h_t2, c_t2 = self.lstm2_locs(h_t, (h_t2, c_t2))
                output_centers = self.dropout(self.fc_locs(h_t2))

                h_t, c_t = self.lstm1_kp(input_kp_t, (h_t, c_t))
                h_t2, c_t2 = self.lstm2_kp(h_t, (h_t2, c_t2))
                output_kp = self.dropout(self.fc_kp(h_t2))
...
class LSTM_PassH(nn.ModuleList):
    def __init__(self, state_dim=3, hidden_dim=256):
        super(LSTM_PassH, self).__init__()
        self.state_dim = state_dim
        self.hidden_dim = hidden_dim

        self.lstm1_kp = nn.LSTMCell(input_size=state_dim+hidden_dim, hidden_size=hidden_dim)
        self.lstm2_kp = nn.LSTMCell(input_size=hidden_dim, hidden_size=hidden_dim)
        self.fc_kp = nn.Linear(in_features=hidden_dim, out_features=state_dim)
...
class LSTM_PassLoc(nn.ModuleList):
    def __init__(self, state_dim=3, hidden_dim=256):
        super(LSTM_PassLoc, self).__init__()
        self.state_dim = state_dim
        self.hidden_dim = hidden_dim

        self.lstm1_kp = nn.LSTMCell(input_size=state_dim+3, hidden_size=hidden_dim)
        self.lstm2_kp = nn.LSTMCell(input_size=hidden_dim, hidden_size=hidden_dim)
        self.fc_kp = nn.Linear(in_features=hidden_dim, out_features=state_dim)
    ...
    def forward(self, x, y, infer=False):
        ...
        for batch_idx in range(len(x)):
            ...
            for input_t_idx in range(len(cur_seq)):
                ...
                kp_input = torch.cat([input_kp_t, output_centers], 1)
                h_t_kp, c_t_kp = self.lstm1_kp(kp_input, (h_t_kp, c_t_kp))
                h_t2_kp, c_t2_kp = self.lstm2_kp(h_t_kp, (h_t2_kp, c_t2_kp))
                output_kp = self.dropout(self.fc_kp(h_t2_kp))
...
def train(dataset, num_epochs=500, observation_length=8, prediction_length=12, batch_size=16, save_path='models/locs_depth/', real_depth=False):
    save_path = save_path + dataset + '/' + str(real_depth) + '/'
    make_dir(save_path)

    data_loader = DataLoader(dataset, batch_size, observation_length, real_depth=real_depth)
    net = LSTM_PassLoc(state_dim=data_loader.state_dim)
...
def test(dataset, observation_length=8, prediction_length=12, batch_size=32, inference_iters=30, save_path='models/locs_depth/', real_depth=False):
    save_path = save_path + dataset + '/' + str(real_depth) + '/'
    make_dir(save_path)
    data_loader = DataLoader(dataset, batch_size, observation_length=observation_length, max_depth=50, real_depth=real_depth)
    net = LSTM_PassH(state_dim=data_loader.state_dim)
    criterion = nn.MSELoss()
...
if __name__ == "__main__":
    ...
    if not args.infer:
        train(args.ds, num_epochs=args.num_epochs, batch_size=args.batch_size, prediction_length=12, real_depth=args.real_depth)
        print('*' * 30)
    x, y, y_hat_lstm, y_hat_fv, y_hat_fa, all_origins, all_meta = test(args.ds, prediction_length=12, real_depth=args.real_depth)
    save_path = 'trajectories/' + args.ds + '/' + str(args.real_depth) + '/'
    make_dir(save_path)
